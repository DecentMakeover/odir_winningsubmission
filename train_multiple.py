...
if dataset_name != 'odir':
    ...
    df_train, df_validation = train_test_split(df, test_size = 0.1, stratify=df[['N','D','G','C','A','H','M','O']], random_state = 73)
...
class_weights=K.variable([[8.77, 8.66, 46.0, 47.,60.0,97.0,57.0,10.0]])
...
model.add(Dense(8,name = 'nelu', activation='sigmoid'))
model.load_weights('saved_models/dr/wieghts_ef5.h5',by_name =True)
model.compile(loss='binary_crossentropy', optimizer=Adam(lr=0.0001), metrics=['mse', 'acc'])
print(model.summary())        
...
if dataset_name =='odir':
    ...
    val_generator = train_datagen.flow_from_dataframe(df_validation, 
                                                      x_col='pic_id', 
                                                      y_col=Labels_list,
                                                      directory = train_images_path,
                                                      target_size=(IMG_WIDTH, IMG_HEIGHT),
...
class_weights={0: 8.77,1: 8.66,2: 46.0,3: 47.0,4:60.0,5: 97.0, 6: 57.0, 7 : 10.0}
